...
parser.add_argument('--N', type=int, default=24, help="Number of object-sampling iterations")
...
for iteration in range(args.N):
    ...
    for obj in objs:
        ...
        while tries < 50:
            ...
            if all(np.linalg.norm(loc - o.get_location()) > 0.07 for o in placed_objs):
                ...
                break
            tries += 1
            print(f"🔍 {len(placed_objs)} objects placed in this iteration.")
    ...
    for _ in range(ANGLES):
        ...
        if not bproc.camera.perform_obstacle_in_view_check(cam_pose, {"min": 0.05}, bvh):
            continue
